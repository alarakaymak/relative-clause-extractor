...
Each cohort folder contains **`results.csv`** (same schema in each).
...
4. **Recommended:** from the **repository root**, run:
   ...
   Then **`result/results.csv`** is written next to **`v2/`** (root-level **`result/`**). Edit **`v2/main.py`** if you want outputs under **`v2/result/`** instead.
...
To compare against an older extraction run, diff on shared keys (e.g. `sent` + `relative_clause`) against **`results.csv`** in each cohort folder.
